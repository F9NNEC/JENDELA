<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Tambah Buku') }}</h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('books.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input name="title" value="{{ old('title') }}" class="mt-1 block w-full border-gray-300 rounded-md" required>
                        @error('title')<div class="text-sm text-red-600">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Author</label>
                        <input name="author" value="{{ old('author') }}" class="mt-1 block w-full border-gray-300 rounded-md" required>
                        @error('author')<div class="text-sm text-red-600">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">ISBN</label>
                        <input name="isbn" value="{{ old('isbn') }}" class="mt-1 block w-full border-gray-300 rounded-md">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="available" min="0" value="{{ old('available', 1) }}" class="mt-1 block w-32 border-gray-300 rounded-md" required>
